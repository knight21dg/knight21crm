  ...
  if ($("#files-upload").length > 0) {
    createDropzone("#files-upload", {
      accept: function (file, done) {
        if (
          $.trim($('#files-upload input[name="subject"]').val()) === "" ||
          $.trim($('#files-upload textarea[name="description"]').val()) === ""
        ) {
          var msg = app.lang.file_subject_and_description_required;
      ...
      sending: function (file, xhr, formData) {
        formData.append("subject", $('#files-upload input[name="subject"]').val());
        formData.append("description", $('#files-upload textarea[name="description"]').val());
      },
...
function customerExternalFileUpload(files, externalType) {
  var subject = $('#files-upload input[name="subject"]').val();
  var description = $('#files-upload textarea[name="description"]').val();

  if ($.trim(subject) === "" || $.trim(description) === "") {
    alert_float("danger", app.lang.file_subject_and_description_required);
    return;
  }
