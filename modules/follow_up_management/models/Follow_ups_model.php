...
class Follow_ups_model extends App_Model
{
    ...
    /**
     * "Today's Calling Summary" - replaces the native Dashboard's Todo
     * widget for a plain Telecaller. Deliberately does NOT reuse
     * get_case_counts()'s 'completed_today' (case_status='closed' AND
     * closed_at=today) for the "Completed Calls Today" figure - that
     * counts ANY case closure today (including a Lead marked Lost, which
     * has nothing to do with a completed call), which would over-count.
     * Instead reuses the existing, already-correct call-specific count
     * this model already computes for the Performance Dashboard
     * (count_history_today('call', $today), see get_todays_performance()
     * below) - both helpers below are private methods of this SAME
     * class, so calling them directly here is genuine reuse, not
     * duplicated query logic. "Pending Calls Today" reuses
     * get_case_counts()'s own 'todays_followups' definition verbatim
     * (open cases due today) - already an exact match for "assigned
     * follow-ups whose Next Follow-up Date = Today and not completed
     * today" (an open case cannot simultaneously be completed today).
     *
     * "Tomorrow's Calls"/"Overdue Calls" (UI expansion, added alongside
     * the 4 keys above without changing their computation) reuse
     * count_cases() with the exact same shape get_case_counts()'s own
     * 'overdue_cases' already uses (case_status=open AND
     * next_follow_up_date < today) for Overdue, and the same shape with
     * tomorrow's date for Tomorrow - not new business logic, just the
     * established open-case-by-date pattern applied to one more date.
     *
     * @return array ['completed_today', 'pending_today', 'total_today', 'completion_percentage', 'tomorrow_calls', 'overdue_calls']
     */
    public function get_todays_calling_summary()
    {
        $today = date('Y-m-d');

        $completed = $this->count_history_today('call', $today);
        $pending   = $this->count_cases(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) = "' . $today . '"');
        ...
        return [
            ...
            'tomorrow_calls'        => $this->count_cases(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) = "' . date('Y-m-d', strtotime('+1 day')) . '"'),
            'overdue_calls'         => $this->count_cases(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) < "' . $today . '"'),
        ];
    }

    /**
     * Detail lists for the Calling Summary widget's expandable "View
     * Details" panel - one caller-name list per category shown in the
     * collapsed summary above, built from the exact same queries/filters
     * that produced those counts (not a re-derivation):
     *   - completed_today: same tblfollowup_history/'call'/today filter
     *     count_history_today() uses, via the sibling get_history_cases_today()
     *     below (COUNT -> SELECT, identical WHERE).
     *   - pending_today/tomorrow_calls/overdue_calls: get_cases_where(),
     *     the same private helper get_todays_followups_grouped() already
     *     uses for its own 'due_today'/'overdue' lists, called here with
     *     the identical $where_equals/$extra_raw pairs used above for the
     *     counts.
     * Names are resolved via get_relation_data()/get_relation_values() -
     * the same generic resolver every other case-list view in this module
     * (todays_followups.php, notifications.php, dashboard.php, widget.php)
     * already uses, not a new lookup.
     *
     * @return array ['completed_today' => [...names], 'pending_today' => [...], 'tomorrow_calls' => [...], 'overdue_calls' => [...]]
     */
    public function get_todays_calling_summary_details()
    {
        $today    = date('Y-m-d');
        $tomorrow = date('Y-m-d', strtotime('+1 day'));

        $case_lists = [
            'completed_today' => $this->get_history_cases_today('call', $today),
            'pending_today'   => $this->get_cases_where(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) = "' . $today . '"', 'next_follow_up_date', 'asc'),
            'tomorrow_calls'  => $this->get_cases_where(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) = "' . $tomorrow . '"', 'next_follow_up_date', 'asc'),
            'overdue_calls'   => $this->get_cases_where(['case_status' => 'open'], 'next_follow_up_date IS NOT NULL AND DATE(next_follow_up_date) < "' . $today . '"', 'next_follow_up_date', 'asc'),
        ];

        $details = [];
        foreach ($case_lists as $key => $cases) {
            $names = [];
            foreach ($cases as $case) {
                $rel_data   = get_relation_data($case->rel_type, $case->rel_id);
                $rel_values = get_relation_values($rel_data, $case->rel_type);
                $names[]    = $rel_values['name'];
            }
            $details[$key] = $names;
        }
