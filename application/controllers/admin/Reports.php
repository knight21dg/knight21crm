...
class Reports extends AdminController
{
    ...
    /**
     * Follow-up Management report - by outcome and by staff. Phase 8
     * extends this in place (Outcome Distribution/Trend, Call Success
     * Rate, Meeting Conversion Rate - the "Follow-up Outcome Report" from
     * the Phase 8 spec) rather than creating a second, overlapping page.
     */
    public function followups()
    {
        $data['title']                       = _l('reports_followups');
        $data['followups_by_outcome_report'] = json_encode($this->reports_model->followups_by_outcome_report());
        $data['followups_by_staff_report']   = json_encode($this->reports_model->followups_by_staff_report());
        $data['followups_outcome_trend_report'] = json_encode($this->reports_model->followups_outcome_trend_report());
        $data['call_success_rate']    = $this->reports_model->followups_call_success_rate();
        $data['meeting_conversion_rate'] = $this->reports_model->followups_meeting_conversion_rate();
        $this->load->view('admin/reports/followups', $data);
    }

    /**
     * @return array Date Range/Department/Staff/Priority/Case Status - the
     *               5 filters every Follow-up report supports, read once
     *               here rather than repeated in each action below.
     */
    private function get_followup_report_filters()
    {
        return [
            ...
            'case_status'   => $this->input->get('case_status') ?: null,
            'date_from_raw' => $this->input->get('date_from'),
            'date_to_raw'   => $this->input->get('date_to'),
        ];
    }

    /**
     * Case Summary Report - Follow_ups_model::get_performance_overview()'s
     * summary (same aggregate the Performance Dashboard's KPI cards use).
     */
    public function followup_case_summary()
    {
        $f                    = $this->get_followup_report_filters();
        $data['summary']      = $this->reports_model->followup_case_summary_report($f['date_from'], $f['date_to'], $f['department_id'], $f['priority'], $f['case_status']);
        $data['filters']      = $f;
        $data['departments']  = get_business_departments();
        $this->load->model('staff_model');
        $data['members']      = $this->staff_model->get('', ['active' => 1]);
        $data['title']        = _l('followup_report_case_summary');
        $this->load->view('admin/reports/followup_case_summary', $data);
    }

    /**
     * Staff Performance Report - full leaderboard, client-side DataTable
     * for Excel/CSV/PDF/Print (see get_datatable_buttons() in the view).
     */
    public function followup_staff_performance()
    {
        $f                    = $this->get_followup_report_filters();
        $data['rows']         = $this->reports_model->followup_staff_performance_report($f['date_from'], $f['date_to'], $f['department_id'], $f['priority'], $f['case_status']);
        $data['filters']      = $f;
        $data['departments']  = get_business_departments();
        $this->load->model('staff_model');
        $data['members']      = $this->staff_model->get('', ['active' => 1]);
        $data['title']        = _l('followup_report_staff_performance');
        $this->load->view('admin/reports/followup_staff_performance', $data);
    }

    /**
     * Department Performance Report - client-side DataTable.
     */
    public function followup_department_performance()
    {
        $f                    = $this->get_followup_report_filters();
        $data['rows']         = $this->reports_model->followup_department_performance_report($f['date_from'], $f['date_to']);
        $data['filters']      = $f;
        $data['departments']  = get_business_departments();
        $data['title']        = _l('followup_report_department_performance');
        $this->load->view('admin/reports/followup_department_performance', $data);
    }

    /**
     * Activity Report - the merged Timeline/Activity Log feed
     * (Follow_ups_model::get_activity_report()), client-side DataTable.
     */
    public function followup_activity_report()
    {
        $f = $this->get_followup_report_filters();
        $this->load->model('follow_up_management/follow_ups_model');
        $data['rows']         = $this->follow_ups_model->get_activity_report($f['date_from'], $f['date_to'], $f['department_id'], $f['staff_id']);
        $data['filters']      = $f;
        $data['departments']  = get_business_departments();
        $this->load->model('staff_model');
        $data['members']      = $this->staff_model->get('', ['active' => 1]);
        $data['title']        = _l('followup_report_activity');
        $this->load->view('admin/reports/followup_activity_report', $data);
    }

    /**
     * SLA Report - Follow_ups_model::get_sla_report() (fixed default
     * thresholds, documented there).
     */
    public function followup_sla_report()
    {
        $f = $this->get_followup_report_filters();
        $this->load->model('follow_up_management/follow_ups_model');
        $data['sla']          = $this->follow_ups_model->get_sla_report($f['date_from'], $f['date_to']);
        $data['filters']      = $f;
        $data['departments']  = get_business_departments();
        $data['title']        = _l('followup_report_sla');
        $this->load->view('admin/reports/followup_sla_report', $data);
    }

    /**
     * Productivity Report - Daily/Weekly/Monthly, entirely reusing the
     * Performance Dashboard's own chart methods (Phase 7) and Today's
     * Performance (Phase 4) - zero new queries.
     */
    public function followup_productivity_report()
    {
        $this->load->model('follow_up_management/follow_ups_model');
        ...
        $data['title']         = _l('followup_report_productivity');
        $this->load->view('admin/reports/followup_productivity_report', $data);
    }

    /**
     * Executive Dashboard Report - a single print-friendly page composing
     * existing Dashboard/Performance methods (Phases 4 and 7); the only
     * report-specific work here is picking top 5 slices and sorting
     * departments, not any new query.
     */
    public function followup_executive_dashboard()
    {
        $this->load->model('follow_up_management/follow_ups_model');
        $overview = $this->follow_ups_model->get_performance_overview();

        $departments = $this->follow_ups_model->get_department_performance_report();
        usort($departments, function ($a, $b) {
            return $b['cases_closed'] <=> $a['cases_closed'];
        });

        $data['summary']              = $overview['summary'];
        $data['top_staff']            = array_slice($overview['leaderboard'], 0, 5);
        $data['top_departments']      = array_slice($departments, 0, 5);
        ...
        $data['upcoming']             = $this->follow_ups_model->get_upcoming_followups(10);
        $data['title']                = _l('followup_report_executive_dashboard');
        $this->load->view('admin/reports/followup_executive_dashboard', $data);
    }
