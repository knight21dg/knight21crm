...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        // Conversion Report KPI cards' initial (unfiltered) counts - rendered
        // once server-side so the cards aren't blank before JS runs, same
        // precedent as this page's existing quick_stats.php summary block.
        // JS takes over from here via Clients::conversion_summary() whenever
        // a report filter changes.
        $this->load->helper('lead_workspace');
        $this->db->select(lead_converted_from_sql_case() . ' as role, COUNT(*) as total', false);
        if (staff_cant('view', 'customers')) {
            $this->db->where('userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id=' . get_staff_user_id() . ')', null, false);
        }
        $this->db->group_by(lead_converted_from_sql_case());
        $conversion_rows = $this->db->get(db_prefix() . 'clients')->result_array();

        $data['conversion_summary'] = ['total' => 0, 'field_executive' => 0, 'telecaller' => 0, 'admin' => 0];
        foreach ($conversion_rows as $row) {
            $data['conversion_summary'][$row['role']] = (int) $row['total'];
            $data['conversion_summary']['total'] += (int) $row['total'];
        }
    ...
    /**
     * KPI cards for the Customer Conversion Report (Total/Field Executive/
     * Telecaller/Admin), refreshed via AJAX whenever the report's filters
     * change - reads ONLY the permanent tblclients.converted_from_role
     * column (COALESCE'd to 'admin' for customers with no Lead involved,
     * same rule the table itself uses via lead_converted_from_sql_case()
     * in application/helpers/lead_workspace_helper.php), never
     * reconstructed from tblleads/tblfollowups. The date-range filter is
     * resolved via the exact same customer_conversion_date_where() the
     * table's own WHERE clause uses, so cards and table always agree.
     */
    public function conversion_summary()
    {
        ...
        $converted_from_filter = $this->input->post('converted_from');
        if (in_array($converted_from_filter, ['field_executive', 'telecaller', 'admin'], true)) {
            $this->db->where(lead_converted_from_sql_case() . " = '" . $converted_from_filter . "'", null, false);
        }

        $date_where = customer_conversion_date_where(
            $this->input->post('converted_date_range'),
            $this->input->post('converted_date_from'),
            $this->input->post('converted_date_to')
        );
        if ($date_where) {
            $this->db->where($date_where, null, false);
        }
        ...
        $counts = ['field_executive' => 0, 'telecaller' => 0, 'admin' => 0];
        $total  = 0;
        foreach ($rows as $row) {
        ...
        echo json_encode([
            'total'           => $total,
            'field_executive' => $counts['field_executive'],
            'telecaller'      => $counts['telecaller'],
            'admin'           => $counts['admin'],
        ]);
    }
    ...
    {
        if (trim($this->input->post('subject')) === '' || trim($this->input->post('description')) === '') {
            header($_SERVER['SERVER_PROTOCOL'] . ' 400 Bad error');
            echo _l('file_subject_and_description_required');
            die;
        }
    ...
    public function add_external_attachment()
    {
        if ($this->input->post()) {
            if (trim($this->input->post('subject')) === '' || trim($this->input->post('description')) === '') {
                ...
                echo json_encode([
                    'success' => false,
                    'message' => _l('file_subject_and_description_required'),
                ]);
                die;
            }

            $files = $this->input->post('files');
            $files[0]['subject']     = $this->input->post('subject');
            $files[0]['description'] = $this->input->post('description');

            $this->misc_model->add_attachment_to_database($this->input->post('clientid'), 'customer', $files, $this->input->post('external'));
        }
    }
