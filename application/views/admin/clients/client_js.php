    ...
    if ($('#client-attachments-upload').length > 0) {
        new Dropzone('#client-attachments-upload', appCreateDropzoneOptions({
            ...
            accept: function(file, done) {
                if ($('#client-attachments-upload input[name="subject"]').val().trim() === '' ||
                    $('#client-attachments-upload textarea[name="description"]').val().trim() === '') {
                    var msg = '<?= _l('file_subject_and_description_required'); ?>';
            ...
            sendingmultiple: function(file, xhr, formData) {
                formData.append('subject', $('#client-attachments-upload input[name="subject"]').val());
                formData.append('description', $('#client-attachments-upload textarea[name="description"]').val());
            },
...
function saveCustomerProfileExternalFile(files, externalType) {
    var subject = $('#client-attachments-upload input[name="subject"]').val();
    var description = $('#client-attachments-upload textarea[name="description"]').val();

    if ($.trim(subject) === '' || $.trim(description) === '') {
        alert_float('danger', '<?= _l('file_subject_and_description_required'); ?>');
        return;
    }
