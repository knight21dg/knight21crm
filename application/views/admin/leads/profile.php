...
?>
            ...
            <div class="col-md-3">
                <?php
                // Next Follow-up Date - not a new field, this is still the
                // existing Lead custom field (tblcustomfields, id looked up
                // by name below, fieldto='leads'). Rendered here beside
                // Status with the generic native date input
                // (render_date_input(), the same renderer used for plain
                // date fields elsewhere, e.g. case.php's "due_date") given
                // this field's own POST name - not render_custom_fields()'s
                // field-editor markup - so it posts to and loads from
                // exactly the same tblcustomfieldsvalues row as before,
                // through the existing generic handle_custom_fields_post()
                // save path, with no backend change required. Excluded
                // from the general custom fields block below (see
                // $next_followup_field_id there) so it isn't rendered
                // twice. Both the field id and the "Follow-up" status id
                // are looked up by name, never hardcoded, since both are
                // install-specific rows.
                ...
                $follow_up_status_id    = 0;
                foreach ($statuses as $_status) {
                    if ($_status['name'] === 'Follow-up') {
                        $follow_up_status_id = $_status['id'];
                        break;
                    }
                }
                $show_next_followup = $next_followup_field_id && (string) $selected === (string) $follow_up_status_id;
                $next_followup_name  = 'custom_fields[leads][' . $next_followup_field_id . ']';
                $next_followup_value = (isset($lead) && $next_followup_field_id)
                    ? get_custom_field_value($lead->id, $next_followup_field_id, 'leads', false)
                    : '';
                ?>
                <?php if ($next_followup_field_id) {
                    echo render_date_input(
                        $next_followup_name,
                        'Next Follow-up Date',
                        $next_followup_value,
                        [],
                        $show_next_followup ? [] : ['style' => 'display:none']
                    );
                } ?>
            </div>
            <div class="col-md-3">
...
echo render_select('assigned', $members, ['staffid', ['firstname', 'lastname']], 'lead_add_edit_assigned', $selected, $assigned_attrs); ?>
            ...
            <div class="col-md-4">
                <?php
                // Business Department - optional. When set, narrows the Assigned
                // dropdown above to that department's staff (see cascade JS
                // below); leaving it blank keeps the full staff list, exactly
                // as leads have always worked.
                $department_options = get_business_departments();
                $selected_lead_department = (isset($lead) ? $lead->department : '');
                echo render_select('department', $department_options, ['id', 'name'], 'business_department', $selected_lead_department);
                ?>
            </div>
            <div class="clearfix"></div>
...
?>
            ...
            <div class="col-md-12 mtop15">
                ...
                <?= render_custom_fields('leads', $rel_id, $next_followup_field_id ? ['id !=' => $next_followup_field_id] : []); ?>
            </div>
            <div class="clearfix"></div>
        </div>
    </div>
...
<script>
    $(function() {
        // Business Department -> Assigned staff filter. Unlike Customers/
        // Projects, the Assigned dropdown is never disabled or emptied here -
        // leads must keep working exactly as before when no department is
        // set, so clearing the department just restores the full staff list.
        var allLeadStaffMembers = <?= json_encode(array_map(function ($m) {
            return ['staffid' => $m['staffid'], 'name' => trim($m['firstname'] . ' ' . $m['lastname'])];
        }, $members)); ?>;
        var leadDepartmentStaffMap = <?= json_encode(get_business_department_staff_map()); ?>;

        $('select[name="department"]').on('change', function() {
            var $assignedSelect = $('select[name="assigned"]');
            var department = $(this).val();
            var employees = (department && leadDepartmentStaffMap[department] && leadDepartmentStaffMap[department].length > 0) ?
                leadDepartmentStaffMap[department] : allLeadStaffMembers;
            var currentVal = $assignedSelect.val();
            ...
            $.each(employees, function(i, employee) {
                $assignedSelect.append($('<option>', {
                    ...
                    text: employee.name,
                    selected: (String(employee.staffid) === String(currentVal))
                }));
            });
