...
return App_table::find('leads')
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $lockAfterConvert = (int) get_option('lead_lock_after_convert_to_customer');
        ...
        // Click-to-call: strips everything except digits and a single
        // leading "+" so "98765-43210" / "+91 98765 43210" become valid
        // tel:/wa.me values - the displayed phone number itself is never
        // touched, only this derived value used for the action links.
        $sanitizePhoneForDialing = function ($number) {
            $digitsAndPlus = preg_replace('/[^0-9+]/', '', $number);

            return (strpos($digitsAndPlus, '+') === 0 ? '+' : '') . str_replace('+', '', $digitsAndPlus);
        };

        // Next Follow-up Date column - not a new field, this is still the
        // existing Lead custom field (tblcustomfields, fieldto='leads'),
        // looked up by name (never hardcoded) since the id is
        // install-specific. show_on_table=0 for this field, so it is not
        // already appearing via the generic $customFieldsColumns loop
        // further down - joined and selected separately here instead so
        // it can sit right after Status rather than at the end of the row.
        ...
        foreach ($rResult as $aRow) {
            ...
            $phoneCell = '';
            if ($aRow['phonenumber'] != '') {
                $phoneCell = '<a href="tel:' . e($aRow['phonenumber']) . '">' . e($aRow['phonenumber']) . '</a>';

                $dialablePhone = $sanitizePhoneForDialing($aRow['phonenumber']);

                if ($dialablePhone !== '') {
                    $phoneCell .= ' <div class="dropdown inline-block lead-phone-actions">';
                    $phoneCell .= '<a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Call"><i class="fa-solid fa-phone"></i></a>';
                    $phoneCell .= '<ul class="dropdown-menu dropdown-menu-right">';
                    $phoneCell .= '<li><a href="tel:' . e($dialablePhone) . '"><i class="fa-solid fa-phone"></i> Call</a></li>';
                    $phoneCell .= '<li><a href="#" class="lead-phone-copy" data-phone="' . e($dialablePhone) . '"><i class="fa-regular fa-copy"></i> ' . _l('copy') . '</a></li>';
                    $phoneCell .= '<li><a href="https://wa.me/' . e($dialablePhone) . '" target="_blank" rel="noopener"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a></li>';
                    $phoneCell .= '</ul>';
                    $phoneCell .= '</div>';
                }
            }
            ...
            // Next Follow-up Date - only meaningful once a lead is in
            // Follow-up status; the value itself still comes straight from
            // the existing custom field (tblcustomfieldsvalues), never
            // duplicated into a new column.
            $row[] = ($aRow['status_name'] === 'Follow-up' && !empty($aRow['next_follow_up_date_value']))
                ? e(_d($aRow['next_follow_up_date_value']))
                : '-';
            ...
            // Removed assigned check to prevent undefined index notice
