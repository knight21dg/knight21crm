...
// Column order: Name | Email | Role | Department | Last Login | Active
$aColumns = [
    'firstname',
    'email',
    db_prefix() . 'roles.name',
    db_prefix() . 'business_departments.name as bd_name',
    'last_login',
...
// Join roles (existing) + Business Departments module
$join = [
    'LEFT JOIN ' . db_prefix() . 'roles ON ' . db_prefix() . 'roles.roleid = ' . db_prefix() . 'staff.role',
    'LEFT JOIN ' . db_prefix() . 'business_department_staff bds_tbl ON bds_tbl.staff_id = ' . db_prefix() . 'staff.staffid',
    'LEFT JOIN ' . db_prefix() . 'business_departments ON ' . db_prefix() . 'business_departments.id = bds_tbl.department_id',
];
...
// Department filter — fired by the UI dropdown via DataTables column search
$department_filter_id = $this->ci->input->post('business_department_id');
if ($department_filter_id !== false && $department_filter_id !== '') {
    $dept_id = (int) $department_filter_id;
    if ($dept_id > 0) {
        // Filter to only staff assigned to that business department
        $where[] = 'AND ' . db_prefix() . 'business_departments.id = ' . $dept_id;
    }
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        // Handle aliased columns like "tbl.col as alias"
        ...
        if ($aColumns[$i] == 'last_login') {
        ...
        } elseif ($aColumns[$i] == 'active' || $aColumns[$i] == db_prefix() . 'staff.active as active') {
            $checked = '';
            if ($aRow['active'] == 1) {
                $checked = 'checked';
            }
            ...
            $_data .= '<a href="' . admin_url('staff/profile_view/' . $aRow['staffid']) . '">' . _l('view') . '</a>';
        ...
        } elseif ($aColumns[$i] == db_prefix() . 'business_departments.name as bd_name') {
            // Department column — display Business Department name or "Not Assigned"
            if (!empty($aRow['bd_name'])) {
                $_data = '<span class="label label-primary">' . e($aRow['bd_name']) . '</span>';
            } else {
                $_data = '<span class="text-muted">Not Assigned</span>';
            }
        } else {
