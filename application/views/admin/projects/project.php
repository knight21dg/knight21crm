...
if ($selected != '') {
    ...
    echo '<option value="' . $rel_val['id'] . '" selected>' . $rel_val['name'] . '</option>';
                            ...
                            <?php
                    // Same reusable Progress dropdown used by the Customers module
                    // (get_progress_options()) - disabled while progress is calculated
                    // from tasks, exactly like the previous slider was.
                    $progress_from_tasks_enabled = (isset($project) && $project->progress_from_tasks == 1) || ! isset($project);
                    ...
                    $selected_progress     = (isset($project) ? $project->progress : 0);
                    $progress_select_attrs = [];
                    if ($progress_from_tasks_enabled) {
                        $progress_select_attrs['disabled'] = 'disabled';
                    }
                    echo render_select('progress', $progress_options, ['id', 'name'], 'project_progress', $selected_progress, $progress_select_attrs);
                    if ($progress_from_tasks_enabled) { ?>
                            <p class="text-muted tw-mt--2">
                                <?= _l('calculate_progress_through_tasks'); ?>: <span class="label_progress"><?= e($value); ?>%</span>
                            </p>
                            <?php } ?>
                            <div class="row">
                                <div class="col-md-6">
...
?>
                            ...
                            <hr />
                            <p class="bold">
                                <?= _l('project_service_section'); ?>
                            </p>
                            <div class="row">
                                <div class="col-md-6">
                                    <?php
                                    // Department dropdown - loads real Business Departments
                                    // (modules/business_departments).
                                    $department_options = get_business_departments();
                                    $selected_department = (isset($project) ? $project->department : '');
                                    echo render_select('department', $department_options, ['id', 'name'], 'project_department', $selected_department);
                                    ?>
                                </div>
                                <div class="col-md-6">
                                    <?php
                                    // Same Department -> Employee dependent dropdown data source as the
                                    // Customers module (get_business_department_staff(), real tblstaff
                                    // records) - no duplicate employee-loading logic.
                                    $selected_assigned_employee = (isset($project) ? $project->assigned_employee : '');
                                    $employee_options = $selected_department != '' ? get_business_department_staff($selected_department) : [];
                                    $employee_select_attrs = [];
                                    if ($selected_department == '') {
                                        $employee_select_attrs['disabled'] = 'disabled';
                                    }
                                    echo render_select('assigned_employee', $employee_options, ['staffid', 'name'], 'project_assigned_employee', $selected_assigned_employee, $employee_select_attrs);
                                    ?>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <span class="text-has-action" data-toggle="tooltip" data-title="<?= e(_dt($project->last_updated)); ?>"><?= e(time_ago($project->last_updated)); ?></span>
                                </div>
                            </div>
                            <?php } ?>
                            <hr />
...
<script>
    ...
    $(function() {
        ...
        // Progress dropdown (same render_select/.selectpicker used by the Customers
        // module) - disabled while progress_from_tasks is checked, matching the
        // previous slider's disabled-while-calculated behavior. bootstrap-select
        // needs an explicit ('refresh') after toggling the native disabled prop.
        var progress_select = $('#progress');
        var progress_from_tasks = $('#progress_from_tasks');

        progress_from_tasks.on('change', function() {
            progress_select.prop('disabled', $(this).prop('checked'));
            progress_select.selectpicker('refresh');
        });

        // Department -> Employee dependent dropdown - same pattern as the
        // Customers module (application/views/admin/clients/client_js.php),
        // adapted to this form's field name (assigned_employee instead of
        // employee_name). Reuses the same departmentEmployeeMap data, now
        // sourced from real tblstaff records via the Business Departments module.
        $('select[name="department"]').on('change', function() {
            var $employeeSelect = $('select[name="assigned_employee"]');
