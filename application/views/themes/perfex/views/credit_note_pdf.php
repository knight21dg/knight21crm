...
if (function_exists('get_bank_details')) {
    ...
    if (! empty($bank_details['bank_name'])) {
        $pdf->Ln(4);
        $pdf->SetFont($font_name, 'B', $font_size);
        $pdf->Cell(0, 0, _l('bank_details_card_heading') . ':', 0, 1, 'L', 0, '', 0);
        $pdf->SetFont($font_name, '', $font_size);
        $pdf->Ln(2);

        $bank_details_fields = [
            'settings_bank_name'            => $bank_details['bank_name'],
            'settings_bank_account_name'    => $bank_details['account_name'],
            'settings_bank_account_number'  => $bank_details['account_number'],
            'settings_bank_ifsc_code'       => $bank_details['ifsc_code'],
            'settings_bank_branch_name'     => $bank_details['branch_name'],
            'settings_bank_swift_code'      => $bank_details['swift_code'],
            'settings_bank_account_type'    => $bank_details['account_type'],
            'settings_bank_upi_id'          => $bank_details['upi_id'],
        ];

        $bank_details_html = '<table cellpadding="2">';
        foreach ($bank_details_fields as $lang_key => $value) {
            if (empty($value)) {
                continue;
            }
            $bank_details_html .= '<tr><td><strong>' . _l($lang_key) . ':</strong></td><td>' . e($value) . '</td></tr>';
        }
        $bank_details_html .= '</table>';

        if (! empty($bank_details['instructions'])) {
            $bank_details_html .= '<div>' . process_text_content_for_display($bank_details['instructions']) . '</div>';
        }

        $bank_details_html .= '<p>' . _l('bank_details_payment_verification_note') . '</p>';

        $pdf->writeHTML($bank_details_html, true, false, false, false, '');

        $qr_filename = get_option('bank_qr_code');
        if ($qr_filename != '') {
            $qr_path = get_upload_path_by_type('company') . $qr_filename;
            if (file_exists($qr_path)) {
                $pdf->Ln(2);
                $pdf->writeHTML('<img width="120px" src="' . $qr_path . '">', true, false, false, false, '');
            }
        }
    }
}
