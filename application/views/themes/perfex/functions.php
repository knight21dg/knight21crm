...
/**
 * Customer Portal display-only progress estimate for a project that has no
 * real progress signal yet (used by views/projects.php - never touches
 * Projects_model::resolve_progress_value(), the shared source of truth also
 * used by the Admin Projects list). Estimates how far along the project
 * looks based on elapsed time between its start date and deadline, the same
 * concept already used for the "time left" indicator on the project detail
 * page, clamped to the given [$min, $max] band so an early-timeline project
 ...
 * @return int
 */
function customer_portal_estimate_progress_from_timeline($start_date, $deadline, $min, $max)
{
    ...
    return (int) round($min + ($elapsed * ($max - $min)));
}
