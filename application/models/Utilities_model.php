...
class Utilities_model extends App_Model
{
    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        if (get_option('show_projects_on_calendar') == 1 && !$ff || $ff && array_key_exists('projects', $filters)) {
            ...
            $this->db->select('name as title,id,clientid, CASE WHEN deadline IS NULL THEN (' . db_prefix() . 'projects.start_date) ELSE deadline END as date,' . get_sql_select_client_company(), false);
            ...
            $this->db->where('status !=', 5);
            $this->db->where("CASE WHEN deadline IS NULL THEN (" . db_prefix() . "projects.start_date BETWEEN '$start' AND '$end') ELSE (deadline BETWEEN '$start' AND '$end') END", null, false);
