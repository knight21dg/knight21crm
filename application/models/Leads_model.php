...
class Leads_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->check_and_trigger_automatic_conversion($insert_id, $data['status']);
    ...
    public function update($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (isset($data['status']) && $current_status_id != $data['status']) {
                ...
                $this->check_and_trigger_automatic_conversion($id, $data['status']);
            }
    ...
    public function update_lead_status($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($current_status != $old_status && $old_status != '') {
                ...
                $this->check_and_trigger_automatic_conversion($data['leadid'], $data['status']);
            }
            $this->db->where('id', $data['leadid']);
            $this->db->update(db_prefix() . 'leads', [
                'last_status_change' => date('Y-m-d H:i:s'),
    ...
    public function auto_convert_lead_to_customer(int $lead_id, $currency =  null): bool|int
    {
        ...
        if ($customer_id) {
            ...
            $this->record_conversion_metadata($lead_id, $customer_id);
    ...
    public function check_and_trigger_automatic_conversion($lead_id, $status_id)
    {
        $status_obj = $this->get_status($status_id);
        if ($status_obj && strtolower($status_obj->name) == 'converted') {
            $lead = $this->get($lead_id);
            if ($lead) {
                // Check if already converted
                ...
                // Check duplicates (same email or same company)
                $existing_client_id = null;
                if (!empty($lead->email)) {
                    $this->db->where('email', $lead->email);
                    $existing_contact = $this->db->get(db_prefix() . 'contacts')->row();
                    if ($existing_contact) {
                        $existing_client_id = $existing_contact->userid;
                    }
                }
                if (!$existing_client_id && !empty($lead->company)) {
                    $this->db->where('company', $lead->company);
                    $existing_client = $this->db->get(db_prefix() . 'clients')->row();
                    if ($existing_client) {
                        $existing_client_id = $existing_client->userid;
                    }
                }

                if ($existing_client_id) {
                    // Update lead as converted only
                    $this->db->where('id', $lead_id);
                    $this->db->update(db_prefix() . 'leads', [
                        'date_converted' => date('Y-m-d H:i:s'),
                    ...
                    $this->log_lead_activity($lead_id, 'not_lead_activity_converted', false, serialize([
                        'System (Duplicate Customer Exists)',
                    ]));
                } else {
                    // Create new customer using Lead data
                    $this->load->model('clients_model');
                    $default_country = get_option('customer_default_country');
                    ...
                    if ($customer_id) {
                        // Insert lead description as note
                        ...
                        $this->log_lead_activity($lead_id, 'not_lead_activity_converted', false, serialize([
                            is_staff_logged_in() ? get_staff_full_name() : 'System',
                        ]));
                    }
                }
            }
    ...
    /**
     * THE permanent source of truth for "which department generated this
     * customer" (converted_from_role) and "who performed the conversion"
     * (converted_by_staff_id), written ONCE, at the single moment they're
     * actually knowable - right when a customer is created from a Lead.
     * Replaces every attempt to reconstruct this later from
     * tblleads/tblfollowups at Customers-list read time
     * (application/helpers/lead_workspace_helper.php) - that
     * reconstruction produced a different answer every time its query
     * changed, even though the underlying data never did, because each
     * version saw a different, incomplete slice of history (some leads
     * get deleted after conversion, Case assignments get reassigned,
     * department membership drifts). Recording the fact at the source
     * removes the need to ever re-derive it.
     *
     * Called from BOTH live Lead->Customer conversion paths -
     * convert_to_customer() (the manual/Field-Portal/Telecaller-auto-
     * convert flows) and auto_convert_lead_to_customer() (the proposal/
     * estimate-acceptance auto-convert flow) - so neither path can leave
     * a customer without this metadata. check_and_trigger_automatic_conversion()
     * above is confirmed dormant (gated on a status name that no longer
     * exists after this engagement's status rename) and is intentionally
     * left untouched.
     *
     * Classification - a Case (tblfollowups, rel_type='lead') exists for
     * this Lead whose responsible staff (assigned_staff_id, falling back
     * to staff_id) is a CONFIRMED member of the Telecalling business
     * department -> 'telecaller'. Otherwise, no Case ever existed (the
     * Lead was converted without ever being handed to a real Telecaller,
     * e.g. an Admin created and converted it directly) - fall back to the
     * Lead's own assigned/addedfrom staff and check THAT for confirmed
     * Field Executing membership -> 'field_executive', or 'admin' if
     * neither department can be confirmed. A directly-created customer
     * (no Lead at all) never calls this method - its columns stay NULL,
     * read as 'admin' by a static default at the query layer, not
     * reconstructed.
     *
     * @param  int $lead_id
     ...
     * @return void
     */
    private function record_conversion_metadata($lead_id, $customer_id)
    {
        $prefix     = db_prefix();
        $tc_dept_id = function_exists('field_portal_telecaller_department_id') ? (int) field_portal_telecaller_department_id() : 0;
        $fe_dept_id = function_exists('field_portal_department_id') ? (int) field_portal_department_id() : 0;

        $this->db->select('1');
        $this->db->where('rel_type', 'lead');
        $this->db->where('rel_id', $lead_id);
        $this->db->where('COALESCE(assigned_staff_id, staff_id) IN (SELECT staff_id FROM ' . $prefix . 'business_department_staff WHERE department_id = ' . $tc_dept_id . ')', null, false);
        $telecaller_case = $this->db->get($prefix . 'followups')->row();

        if ($telecaller_case) {
            $role = 'telecaller';
        } else {
            // No Case ever existed for this Lead (e.g. an Admin created and
            // converted it directly, without it ever passing through
            // Follow-up Management) - the old unconditional "field_executive"
            // fallback here mislabeled every such conversion, since it never
            // actually confirmed Field Executing department involvement.
            // Mirror the same confirmed-membership check used for telecaller
            // above, keyed off the Lead's own assigned/addedfrom staff.
            $this->db->select('assigned, addedfrom');
            $this->db->where('id', $lead_id);
            $lead                  = $this->db->get($prefix . 'leads')->row();
            $responsible_staff_id  = $lead ? ($lead->assigned ?: $lead->addedfrom) : null;

            $is_field_executive = false;
            if ($responsible_staff_id && $fe_dept_id) {
                $this->db->select('1');
                $this->db->where('staff_id', $responsible_staff_id);
                $this->db->where('department_id', $fe_dept_id);
                $is_field_executive = (bool) $this->db->get($prefix . 'business_department_staff')->row();
            }

            $role = $is_field_executive ? 'field_executive' : 'admin';
        }

        $this->db->where('userid', $customer_id);
        $this->db->update($prefix . 'clients', [
            'converted_from_role'   => $role,
            'converted_by_staff_id' => get_staff_user_id(),
            'converted_at'          => date('Y-m-d H:i:s'),
        ]);
    }

    /**
     * The canonical Lead -> Customer conversion logic, extracted verbatim
     * from Leads::convert_to_customer() (application/controllers/admin/
     * Leads.php) so it has exactly one implementation shared by both the
     * human-facing "Convert to Customer" form and any automated caller
     * (see modules/follow_up_management's Follow_ups_model::
     * auto_convert_confirmed_lead()) - neither auto_convert_lead_to_customer()
     * nor check_and_trigger_automatic_conversion() above fire the
     * lead_converted_to_customer hook, which is why this extraction exists
     * instead of a third divergent "auto convert" method.
     *
     * $data must already be shaped exactly as the controller shapes it
     * before calling this (billing_* mirrored from address/city/state/zip/
     * country, is_primary=1, leadid present) - this method does not
     * re-derive or validate that shape, matching the original code's own
     * lack of server-side validation.
     *
     * @param array       $data                         Customer/contact fields, POST-shaped
     * @param string      $original_lead_email
     * @param array|null  $notes                        Lead notes to transfer, or null to skip
     * @param array|null  $consents                     GDPR consents to transfer, or null to skip
     * @param array|null  $merge_db_fields              fieldid => tblclients column, for custom-field merge type 2
     * @param array|null  $merge_db_contact_fields      fieldid => tblcontacts column, for custom-field merge type 3
     * @param array|null  $include_leads_custom_fields  fieldid => merge-type (1-5), or null to skip entirely
     *
     * @return int|false New customer id, or false if customer creation failed
     */
    public function convert_to_customer($data, $original_lead_email, $notes = null, $consents = null, $merge_db_fields = null, $merge_db_contact_fields = null, $include_leads_custom_fields = null)
    {
        $this->load->model('clients_model');
        ...
        if ($id) {
            // Recorded immediately, before the GDPR delete-after-convert
            // branch below can remove the Lead row - record_conversion_metadata()
            // needs the Lead's Case history to still be queryable.
            $this->record_conversion_metadata($data['leadid'], $id);
            ...
            $this->log_lead_activity($data['leadid'], 'not_lead_activity_converted', false, serialize([
                get_staff_full_name(),
            ]));
            $default_status = $this->get_status('', [
                'isdefault' => 1,
            ]);
            $this->db->where('id', $data['leadid']);
            $this->db->update(db_prefix() . 'leads', [
            ...
            if ($contact->email != $original_lead_email) {
                if ($original_lead_email != '') {
                    $this->log_lead_activity($data['leadid'], 'not_lead_activity_converted_email', false, serialize([
                        $original_lead_email,
                        $contact->email,
                    ]));
                }
            }
            if ($include_leads_custom_fields) {
