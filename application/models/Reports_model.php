...
class Reports_model extends App_Model
{
    ...
    /**
     * Follow-up Management - count of logged follow-ups per outcome
     * @return array chart data
     */
    public function followups_by_outcome_report()
    {
        $outcomes = get_followup_outcomes();
        $colors   = get_system_favourite_colors();

        $chart = [
            'labels'   => array_values($outcomes),
            'datasets' => [
                [
                    'data'                 => [],
                    'backgroundColor'      => [],
        ...
        foreach ($outcomes as $key => $label) {
            $chart['datasets'][0]['data'][] = total_rows(db_prefix() . 'followups', ['outcome' => $key]);
    ...
    /**
     * Follow-up Management - count of logged follow-ups per staff member
     * @return array chart data
     */
    public function followups_by_staff_report()
    {
        $this->load->model('staff_model');
        ...
        foreach ($staff as $member) {
            $chart['labels'][]              = $member['firstname'] . ' ' . $member['lastname'];
            $chart['datasets'][0]['data'][] = total_rows(db_prefix() . 'followups', ['staff_id' => $member['staffid']]);
        }
    ...
    /**
     * =====================================================================
     * FOLLOW-UP MANAGEMENT - Phase 8 reports. The Case Summary/Staff
     * Performance/Department Performance/Productivity/Executive Dashboard
     * reports are thin delegations to Follow_ups_model, which already
     * computes this exact data for the module's own Dashboard/Performance
     * pages (Phases 4 and 7) - reused here, not recomputed. Outcome Trend/
     * Call Success/Meeting Conversion below are the only genuinely new
     * queries, extending followups_by_outcome_report() above with a time
     * dimension and two rate calculations.
     * =====================================================================
     */

    /**
     * Follow-up Outcome Trend (Line/Bar) - weekly outcome counts, stacked,
     * last $weeks weeks. Extends the existing Outcome report above with a
     * time dimension it didn't have.
     *
     * @param int $weeks
     * @return array Chart.js data
     */
    public function followups_outcome_trend_report($weeks = 8)
    {
        $labels      = [];
        $week_starts = [];
        ...
        foreach ($outcomes as $key => $label) {
            ...
            foreach ($week_starts as $start) {
                ...
                $this->db->where('outcome', $key);
                $this->db->where('follow_up_date >=', $start . ' 00:00:00');
                $this->db->where('follow_up_date <=', $end . ' 23:59:59');
                ...
                $data[] = (int) $this->db->count_all_results(db_prefix() . 'followups');
            }
    ...
    /**
     * Call Success Rate - % of logged calls (tblfollowup_history,
     * activity_type='call') whose outcome is 'interested' or 'converted'.
     *
     * @param string|null $date_from
     * @param string|null $date_to
     * @return array
     */
    public function followups_call_success_rate($date_from = null, $date_to = null)
    {
        $this->db->select(db_prefix() . 'followup_history.outcome as history_outcome', false);
        ...
        $this->db->where(db_prefix() . 'followup_history.activity_type', 'call');
        $this->db->where(db_prefix() . 'followup_history.outcome IS NOT NULL', null, false);

        if ($date_from) {
            $this->db->where(db_prefix() . 'followup_history.event_datetime >=', $date_from . ' 00:00:00');
        }
        if ($date_to) {
            $this->db->where(db_prefix() . 'followup_history.event_datetime <=', $date_to . ' 23:59:59');
        }
        ...
        $success = 0;
        foreach ($rows as $row) {
            if (in_array($row->history_outcome, ['interested', 'converted'], true)) {
                $success++;
            }
        }
        ...
        return [
            'total'   => $total,
            'success' => $success,
            'rate'    => $total > 0 ? round(($success / $total) * 100, 1) : 0.0,
        ];
    }

    /**
     * Meeting Conversion Rate - % of cases that had at least one logged
     * meeting (tblfollowup_history, activity_type='meeting') and either
     * closed or reached outcome='converted'. Grouped by case (not by
     * meeting row) so a case with several meetings only counts once.
     *
     * @param string|null $date_from
     * @param string|null $date_to
     * @return array
     */
    public function followups_meeting_conversion_rate($date_from = null, $date_to = null)
    {
        $this->db->select(db_prefix() . 'followup_history.followup_id, ' . db_prefix() . 'followups.case_status, ' . db_prefix() . 'followups.outcome as case_outcome', false);
        $this->db->from(db_prefix() . 'followup_history');
        $this->db->join(db_prefix() . 'followups', db_prefix() . 'followups.id = ' . db_prefix() . 'followup_history.followup_id');
        $this->db->where(db_prefix() . 'followup_history.activity_type', 'meeting');
        ...
        $this->db->group_by(db_prefix() . 'followup_history.followup_id');
        $rows = $this->db->get()->result();

        $total     = count($rows);
        $converted = 0;
        foreach ($rows as $row) {
            if ($row->case_status === 'closed' || $row->case_outcome === 'converted') {
                $converted++;
            }
        }
    ...
    /**
     * Case Summary Report - delegates to Follow_ups_model::get_performance_overview()'s
     * summary (the same aggregate the Performance Dashboard's KPI cards use,
     * Phase 7), not a new query.
     */
    public function followup_case_summary_report($date_from = null, $date_to = null, $department = null, $priority = null, $status = null)
    {
        $this->load->model('follow_up_management/follow_ups_model');

        return $this->follow_ups_model->get_performance_overview($date_from, $date_to, $department, $priority, $status)['summary'];
    }

    /**
     * Staff Performance Report - delegates to Follow_ups_model::get_performance_overview(),
     * uncapped (the Performance Dashboard's own Leaderboard caps at 10 for
     * a dashboard widget; this report shows every in-scope staff member).
     */
    public function followup_staff_performance_report($date_from = null, $date_to = null, $department = null, $priority = null, $status = null)
    {
        $this->load->model('follow_up_management/follow_ups_model');

        return $this->follow_ups_model->get_performance_overview($date_from, $date_to, $department, $priority, $status, 9999)['leaderboard'];
    }

    /**
     * Department Performance Report - delegates to
     * Follow_ups_model::get_department_performance_report() (the one
     * genuinely new Phase 8 query for this report).
     */
    public function followup_department_performance_report($date_from = null, $date_to = null)
    {
        $this->load->model('follow_up_management/follow_ups_model');

        return $this->follow_ups_model->get_department_performance_report($date_from, $date_to);
    }
