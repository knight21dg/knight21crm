...
class Clients_model extends App_Model
{
    ...
    /**
     * Inline-update a single Customer Assignment field from the Customer list
     * (Department / Employee / Work Status). Single reusable method behind
     * the AJAX quick-edit dropdowns - avoids a dedicated endpoint/method per
     * field.
     *
     * @param  integer $id
     * @param  string  $field
     ...
     *                  untouched otherwise, so a user who has already
     *                  progressed past 25% keeps being able to raise it
     *                  manually without this rule fighting them.
     *   On Hold     -> unchanged (explicit no-op).
     *   Completed   -> 100
     *   Cancelled   -> 0
     *   Anything else (defensive default) -> unchanged.
     *
     * @param string $work_status     the new Work Status value
     * @param int    $current_progress the customer's progress before this change
     * @return int
     */
    private function resolve_progress_for_work_status($work_status, $current_progress)
    {
        return resolve_client_progress_for_status($work_status, $current_progress);
    }

    /**
     * Applies resolve_progress_for_work_status() to $data by reference,
     * shared by both write paths (see that method's docblock). Only acts
     * when `work_status` is present in $data AND genuinely differs from
     * the customer's current stored value - critical for the Customer
     * Edit page, whose form always resubmits work_status alongside
     * progress on every save even when the user only touched the
     * Progress field; without this guard, resaving the same Work Status
     * would silently clobber a manually-typed Progress value back to
     * whatever the rule computes, breaking "existing manual progress
     * editing should continue to work while status is In Progress."
     *
     * @param array $data by reference - 'progress' is added/overwritten
     *                     in place when the rule applies
     * @param mixed $id   customer id
     * @return void
     */
    private function sync_progress_with_work_status(&$data, $id)
    {
        if (!isset($data['work_status'])) {
            return;
        }

        $current = $this->db->select('work_status, progress')->where('userid', $id)->get(db_prefix() . 'clients')->row();

        if (!$current || $current->work_status === $data['work_status']) {
            return;
        }
