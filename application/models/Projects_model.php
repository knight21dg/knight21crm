...
class Projects_model extends App_Model
{
    ...
    {
        $this->db->select('id,progress_from_tasks,progress,status');
        $this->db->where('id', $id);
        $project = $this->db->get(db_prefix() . 'projects')->row();

        return $this->resolve_progress_value($project);
    }

    /**
     * Single source of truth for the progress percentage to display for an
     * already-fetched project row (id/status/progress/progress_from_tasks) -
     * reused by calc_progress() above, the Admin Projects list, and the
     * Customer Portal Projects list, so all three always show the exact
     * same value with no duplicated branching logic. Accepts either an
     * array (DataTables rows, portal list rows) or an object.
     *
     * @param  array|object $row
     * @return int
     */
    public function resolve_progress_value($row)
    {
        $row = (object) $row;
        ...
        if ($row->progress_from_tasks == 1) {
            return (int) $this->calc_progress_by_tasks($row->id);
        }
    ...
    public function get($id = '', $where = [])
    {
        ...
        // Explicitly scoped to tblprojects.* (rather than a bare *) so that
        // columns of the same name on the joined tblclients table (e.g.
        // department, assigned_work, status_description, progress,
        // last_updated, start_date) can never silently overwrite this
        // project's own values in the resulting associative array.
        $this->db->select(db_prefix() . 'projects.*,' . get_sql_select_client_company());
    ...
    /**
     * Inline AJAX quick-edit for Project Assignment fields from the Projects
     * list (Department / Assigned Employee / Assigned Work / Work Status).
     * Mirrors Clients_model::update_assignment_field(). Deliberately does NOT
     * touch the core `status` column - that field has its own dedicated
     * flow (see mark_as() below) with side effects (notifications,
     * date_finished, task-completion cascades) that a raw column update
     * must not bypass.
     *
     * @param  integer $id
     * @param  string  $field
    ...
    public function update_assignment_field($id, $field, $value)
    {
        ...
        // 'progress' reuses this same narrow, single-column update rather
        // than the full Projects_model::update($data, $id) method - that
        // method assumes the complete admin edit-form payload (it resets
        // every project_settings row to disabled whenever $data['settings']
        // is absent, and reads $data['status'] unconditionally), so calling
        // it here with a minimal ['progress' => x] array would silently
        // wipe the project's feature-tab settings. Clamped 0-100 since
        // this method is now reachable from the Development Portal's
        // member-level (not admin-only) context, not just this file's
        // original admin-only inline-edit caller.
        if ($field === 'progress') {
        ...
        // be cleared whenever Department changes.
        if ($field == 'department') {
        ...
        if ($field === 'assigned_employee' && !empty($value)) {
            ...
            if (!$exists) {
                ...
                if ($this->db->affected_rows() > 0) {
                    $this->log_activity($id, 'project_activity_added_team_member', get_staff_full_name($value));
                }
            }
        }
