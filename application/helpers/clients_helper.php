...
/**
 * Single reusable source of truth for the Work Status color scheme,
 * reused for both the inline list dropdown and anywhere else a
 * status color needs rendering.
 *
 * @param string $status
 *
 * @return string hex color
 */
function get_work_status_color($status) {
    $colors = [
        'Pending'     => '#eab308',
        'In Progress' => '#3b82f6',
        'On Hold'     => '#f97316',
 ...
 * @return array
 */
function get_progress_options() {
    return range(0, 100, 10);
}

/**
 * Single reusable source for the Due Date color class (list and anywhere
 * else the same coloring convention is needed).
 *
 * Red only when overdue AND not completed - normal styling otherwise.
 *
 * @param string|null $due_date
 * @param string|null $work_status
 *
 * @return string
 */
function get_due_date_class($due_date, $work_status) {
    if (empty($due_date) || $work_status == 'Completed') {
        return '';
    }
...
/**
 * Single source of truth for what a customer's Progress should be given
 * its Work Status - shared by Clients_model's write-time sync (only
 * applied when work_status actually changes, to preserve manual progress
 * edits while status stays the same) and the Customers list's display
 * layer (applied on every render, so a row whose stored progress has
 * drifted out of sync with its status - e.g. legacy data from before
 * this rule existed - always renders correctly without needing a data
 * migration first).
 *
 * @param string|null $work_status
 * @param int         $progress    the customer's currently stored progress
 *
 * @return int
 */
function resolve_client_progress_for_status($work_status, $progress) {
    switch ($work_status) {
        case 'Pending':
        case 'Cancelled':
            return 0;
        case 'In Progress':
            return ((int) $progress === 0) ? 25 : (int) $progress;
        case 'Completed':
            return 100;
        case 'On Hold':
        default:
...
/**
 * Single reusable source for the Progress bar color (list display).
 * Reuses this fork's existing color palette (migration 342 service-type
 ...
 * @return string hex color
 */
function get_progress_bar_color($progress) {
    if ($progress <= 0) {
        return '#e5e7eb';
    }
